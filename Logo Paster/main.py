...
class App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #Start Process
        self.lbl_start = LabelFrame(self,bg='#005082',fg="#fff",relief=tk.RIDGE,labelanchor="n")
        self.lbl_start.grid(row=3,column=0,padx=self.xx//4)
        self.start_btn = Button(self.lbl_start,bg='#005082',fg="#fff", text='Start Process',
                                     command=self.start_process,relief=tk.RAISED)
        self.start_btn.grid(sticky="n e w s", row=0)
    ...
    def start_process(self):
        ...
        self.progress_lbl= Label(self.lbl_progress,bg='#005082',fg="#fff", text='Task in Progress...')
        ...
        if(Flag):
            time.sleep(1)
            self.progress_lbl.config(text="Please select an Image source directory first!",width=40)
            Flag=0
        else:
            # self.progress_lbl.grid_remove()
            self.progress_lbl.config(text='Task Completed!',width=40)
    ...
    def open_wtrmk_dir(self):
        ...
        if(self.wtrmk_from_user==""):
            ...
            os.rename(f"{self.dstn_photos_dir}/{os.path.basename(self.wtrmk_from_user)}",f"{os.path.dirname(self.dstn_photos_dir)}/wtrmk_dir/logo.png")
...
class AutomatePhotoshopWatermark(App):
    def __init__(self, *args, **kwargs) :
        ...
        self.wtrmk_img = Image.open(f"{os.getcwd()}/wtrmk_dir/logo.png")
